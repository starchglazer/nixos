{
  dandelion.modules.intel = { pkgs, lib, config, ... }:
  let
    inherit (lib) mkIf getExe mkOption;
    inherit (lib.types) enum listOf package;

    cfg = config.opt-ins.graphics.intel;
  in {
    options.opt-ins.graphics.intel = {
    ...
    config = {
      ...
      # boot.kernelParams = [ "i915.enable_fbc=1" ]; # unsure what this does
